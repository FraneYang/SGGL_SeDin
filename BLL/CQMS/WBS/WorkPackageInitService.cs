...
namespace BLL
{
    public class WorkPackageInitService
    {
        ...
        {
            return BLL.Funs.DB.WBS_WorkPackageInit.FirstOrDefault(x => x.WorkPackageCode == workPackageCode);
        }
        ...
        {
            return (from x in BLL.Funs.DB.WBS_WorkPackageInit where x.SuperWorkPack == workPackageCode select x).ToList();
        }
    }
}
