...
namespace BLL
{
    public static class Project_SysSetService
    {
        ...
        {
            var q = from x in new Model.SGGLDB(Funs.ConnString).Project_Sys_Set where x.SetId == setId && x.ProjectId == projectId select x;
            return q.First().IsAuto;
        }

        public static Model.Project_Sys_Set GetSysSetBySetId(string setId, string projectId)
        {
            return new Model.SGGLDB(Funs.ConnString).Project_Sys_Set.Where(x => x.SetId == setId && x.ProjectId == projectId).FirstOrDefault();
        }
        public static Model.Project_Sys_Set GetSysSetBySetName(string setName, string projectId)
        {
            return new Model.SGGLDB(Funs.ConnString).Project_Sys_Set.Where(x => x.SetName == setName && x.ProjectId == projectId).FirstOrDefault();
        }
        /// <summary>
        /// 增加系统变量
        /// </summary>
