    ...
    public static class APISeDinMonthReportService
    {
        ...
        public static int count(string monthReportId)
        {
            using (Model.SGGLDB db = new Model.SGGLDB(Funs.ConnString))
            {
                ...
                var count =  new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport.Where(p=>p.MonthReportId== monthReportId).Count();
                return count;
             }
        }
        ...
        {
            using (Model.SGGLDB db = new Model.SGGLDB(Funs.ConnString)) { 
               return new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport.FirstOrDefault(p => p.MonthReportId == monthReportId);
            }
        }
        ...
        public static List<Model.SeDinMonthReportItem> getSeDinMonthReportList(string projectId, string month, string states)
        {
            ...
            var getSeDinMonthReport = from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport
            ...
            var getReport = from x in getSeDinMonthReport
                            ...
                            select new Model.SeDinMonthReportItem
                            {
                                ...
                                AuditManName = new Model.SGGLDB(Funs.ConnString).Sys_User.First(u => u.UserId == x.AuditManId).UserName,
                                ApprovalManId = x.ApprovalManId,
                                ApprovalManName = new Model.SGGLDB(Funs.ConnString).Sys_User.First(u => u.UserId == x.ApprovalManId).UserName,
        ...
        public static Model.SeDinMonthReport1Item getSeDinMonthReportNullPage1(string projectId)
        {
            ...
            var m = new Model.SGGLDB(Funs.ConnString).Project_ProjectUser.FirstOrDefault(x => x.ProjectId == projectId && x.RoleId.Contains(Const.ProjectManager));
            if (m != null)
            {
            ...
            var h = new Model.SGGLDB(Funs.ConnString).Project_ProjectUser.FirstOrDefault(x => x.ProjectId == projectId && x.RoleId.Contains(Const.HSSEManager));
            if (h != null)
            {
                hsseManagerId = h.UserId;
            }
            var newItem = from x in new Model.SGGLDB(Funs.ConnString).Base_Project
                          where x.ProjectId == projectId
                          select new Model.SeDinMonthReport1Item
                          {
                              ProjectCode = x.ProjectCode,
                              ProjectName = x.ProjectName,
                              ProjectType=new Model.SGGLDB(Funs.ConnString).Sys_Const.First(y=>y.GroupId == ConstValue.Group_ProjectType && y.ConstValue==x.ProjectType).ConstText,
                              StartDate=string.Format("{0:yyyy-MM-dd}",x.StartDate),
                              EndDate = string.Format("{0:yyyy-MM-dd}", x.EndDate),
        ...
        public static Model.SeDinMonthReport2Item getSeDinMonthReportNullPage2(string projectId, string month,string startDate, string endDate)
        {
            ...
            if (getProject != null )
            {
                #region 从人工时月报 取值
                ////// 人工时月报
                //var getProjectMonthReport = from x in new Model.SGGLDB(Funs.ConnString).SitePerson_MonthReport where x.ProjectId == projectId select x;
                //if (getProjectMonthReport.Count() > 0 && monthD.HasValue)
                //{
                //    ////人工时月报明细
                //    var getMonthReportDetail = from x in new Model.SGGLDB(Funs.ConnString).SitePerson_MonthReportDetail
                ...
                var getAllNum = new Model.SGGLDB(Funs.ConnString).SitePerson_PersonInOutNumber.Where(x => x.ProjectId == projectId);
        ...
        public static List<Model.SeDinMonthReport3Item> getSeDinMonthReport3ItemNull(string projectId, string month, string startDate, string endDate)
        {
            ...
            var getAccidentReportTypes= from x in new Model.SGGLDB(Funs.ConnString).Sys_Const
                                        where x.GroupId == ConstValue.Group_AccidentReportRegistration
                                        select x;
            var getAccident = from x in new Model.SGGLDB(Funs.ConnString).Accident_AccidentReport
                              where x.ProjectId == projectId 
                              select x;
        ...
        public static List<Model.SeDinMonthReport4Item> getSeDinMonthReportNullPage4(string projectId, string month, string startDate, string endDate)
        {
            ...
            var getUnits = from x in new Model.SGGLDB(Funs.ConnString).Base_Unit
        ...
        public static List<Model.SeDinMonthReport5Item> getSeDinMonthReportNullPage5(string projectId, string month, string startDate, string endDate)
        {
            ...
            var getUnits = from x in new Model.SGGLDB(Funs.ConnString).Base_Unit
                           ...
                           where y.ProjectId == projectId
                           select x;
            var getAll = from x in new Model.SGGLDB(Funs.ConnString).QualityAudit_EquipmentQuality                       
        ...
        public static Model.SeDinMonthReport7Item getSeDinMonthReportNullPage7(string projectId, string month, string startDate, string endDate)
        {
            var startDateD = Funs.GetNewDateTime(startDate);
            var endDateD = Funs.GetNewDateTime(endDate);
            var getTrainRecordAll = from x in new Model.SGGLDB(Funs.ConnString).EduTrain_TrainRecord
                                    where x.ProjectId == projectId
                                    select x;
        ...
        public static Model.SeDinMonthReport8Item getSeDinMonthReportNullPage8(string projectId, string month, string startDate, string endDate)
        {
            ...
            //// 周例会
            var getAllWeeks = from x in new Model.SGGLDB(Funs.ConnString).Meeting_WeekMeeting where x.ProjectId == projectId select x;
            ...
            /// 月例会
            var getAllMonths = from x in new Model.SGGLDB(Funs.ConnString).Meeting_MonthMeeting where x.ProjectId == projectId select x;
            ...
            /// 专题会议
            var getAllSpecials = from x in new Model.SGGLDB(Funs.ConnString).Meeting_SpecialMeeting where x.ProjectId == projectId select x;
        ...
        public static List<Model.SeDinMonthReport8ItemItem> getSeDinMonthReport8ItemNull(string projectId, string month, DateTime? startDateD, DateTime? endDateD)
        {
            ...
            var getUnits = from x in new Model.SGGLDB(Funs.ConnString).Base_Unit
                                         ...
                                         where y.ProjectId == projectId
                                         select x;
            var getClassMeets = from x in new Model.SGGLDB(Funs.ConnString).Meeting_ClassMeeting
                                where x.ProjectId == projectId && x.ClassMeetingDate >=startDateD && x.ClassMeetingDate <endDateD
                                select x;
            foreach (var item in getUnits)
            {
                var getTeamGroups = from x in new Model.SGGLDB(Funs.ConnString).ProjectData_TeamGroup
        ...
        public static Model.SeDinMonthReport9Item getSeDinMonthReportNullPage9(string projectId, string month, string startDate, string endDate)
        {
            var startDateD = Funs.GetNewDateTime(startDate);
            var endDateD = Funs.GetNewDateTime(endDate);
            var getHazardRegistersAll = from x in new Model.SGGLDB(Funs.ConnString).HSSE_Hazard_HazardRegister
                                        where x.ProjectId == projectId
                                        select x;
            ...
            var getCheckSpecialAll = from x in new Model.SGGLDB(Funs.ConnString).Check_CheckSpecial
                                        where x.ProjectId == projectId
                                        select x;
            ...
            var getCheckColligationAll = from x in new Model.SGGLDB(Funs.ConnString).Check_CheckColligation
                                     where x.ProjectId == projectId
                                     select x;
        ...
        public static List<Model.SeDinMonthReport9ItemRectification> getSeDinMonthReport9ItemRectificationNull(string projectId, string month, string startDate, string endDate)
        {
            ...
            var getUnits = from x in new Model.SGGLDB(Funs.ConnString).Base_Unit
                           ...
                           select x;
            var getAll = from x in new Model.SGGLDB(Funs.ConnString).Check_RectifyNotices where x.ProjectId == projectId select x;
        ...
        public static List<Model.SeDinMonthReport9ItemSpecial> getSeDinMonthReport9ItemSpecialNull(string projectId, string month, string startDate, string endDate)
        {
            ...
            var getCheckSpecialAll = from x in new Model.SGGLDB(Funs.ConnString).Check_CheckSpecialDetail
                                     join y in new Model.SGGLDB(Funs.ConnString).Check_CheckSpecial  on x.CheckSpecialId equals y.CheckSpecialId
                                     where y.ProjectId == projectId
                                     select x;
            var getCheckSpecialYear = from x in getCheckSpecialAll
                                      join y in new Model.SGGLDB(Funs.ConnString).Check_CheckSpecial on x.CheckSpecialId equals y.CheckSpecialId
        ...
        public static List<Model.SeDinMonthReport9ItemStoppage> getSeDinMonthReport9ItemStoppageNull(string projectId, string month, string startDate, string endDate)
        {
            ...
            var getUnits = from x in new Model.SGGLDB(Funs.ConnString).Base_Unit
                           ...
                           where y.ProjectId == projectId && y.UnitType == Const.ProjectUnitType_2
                           select x;
            var getAll = from x in new Model.SGGLDB(Funs.ConnString).Check_PauseNotice where x.ProjectId == projectId select x;
        ...
        public static Model.SeDinMonthReport10Item getSeDinMonthReportNullPage10(string projectId, string month, string startDate, string endDate)
        {
            ...
            var getIncentiveNoticeAll = from x in new Model.SGGLDB(Funs.ConnString).Check_IncentiveNotice where x.ProjectId == projectId select x;
            ...
            var getPunishNoticeAll = from x in new Model.SGGLDB(Funs.ConnString).Check_PunishNotice where x.ProjectId == projectId select x;
        ...
        public static Model.SeDinMonthReport11Item getSeDinMonthReportNullPage11(string projectId, string month, string startDate, string endDate)
        {
            var startDateD = Funs.GetNewDateTime(startDate);
            var endDateD = Funs.GetNewDateTime(endDate);
            var getLargerHazardAll = from x in new Model.SGGLDB(Funs.ConnString).Solution_LargerHazard where x.ProjectId == projectId select x;
        ...
        public static Model.SeDinMonthReportItem getSeDinMonthReport0ById(string projectId, string month)
        {
            ...
            if (monthD.HasValue)
            {
                getInfo = (from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport
        ...
        public static Model.SeDinMonthReport1Item getSeDinMonthReport1ById(string projectId, string month)
        {
            ...
            if (monthD.HasValue)
            {
                getInfo = (from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport1
                           join y in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport on x.MonthReportId equals y.MonthReportId
        ...
        public static Model.SeDinMonthReport2Item getSeDinMonthReport2ById(string projectId, string month)
        {
            ...
            if (monthD.HasValue)
            {
                getInfo = (from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport2
                           join y in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport on x.MonthReportId equals y.MonthReportId
                           where y.ProjectId == projectId && y.ReporMonth.Value.Year == monthD.Value.Year && y.ReporMonth.Value.Month == monthD.Value.Month
        ...
        {
            var getInfo = from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport3
        ...
        public static List<Model.SeDinMonthReport4Item> getSeDinMonthReport4ById(string projectId, string month)
        {
            ...
            if (monthD.HasValue)
            {
                getInfo = (from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport4
                           join y in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport on x.MonthReportId equals y.MonthReportId
                           where y.ProjectId == projectId && y.ReporMonth.Value.Year == monthD.Value.Year && y.ReporMonth.Value.Month == monthD.Value.Month
        ...
        public static List<Model.SeDinMonthReport5Item> getSeDinMonthReport5ById(string projectId, string month)
        {
            ...
            if (monthD.HasValue)
            {
                getInfo = (from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport5
                           join y in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport on x.MonthReportId equals y.MonthReportId
                           where y.ProjectId == projectId && y.ReporMonth.Value.Year == monthD.Value.Year && y.ReporMonth.Value.Month == monthD.Value.Month
        ...
        public static Model.SeDinMonthReport6Item getSeDinMonthReport6ById(string projectId, string month)
        {
            ...
            if (monthD.HasValue)
            {
                getInfo = (from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport6
                           join y in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport on x.MonthReportId equals y.MonthReportId
                           where y.ProjectId == projectId && y.ReporMonth.Value.Year == monthD.Value.Year && y.ReporMonth.Value.Month == monthD.Value.Month
        ...
        public static Model.SeDinMonthReport7Item getSeDinMonthReport7ById(string projectId, string month)
        {
            ...
            if (monthD.HasValue)
            {
                getInfo = (from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport7
                           join y in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport on x.MonthReportId equals y.MonthReportId
                           where y.ProjectId == projectId && y.ReporMonth.Value.Year == monthD.Value.Year && y.ReporMonth.Value.Month == monthD.Value.Month
        ...
        public static Model.SeDinMonthReport8Item getSeDinMonthReport8ById(string projectId, string month)
        {
            ...
            if (monthD.HasValue)
            {
                getInfo = (from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport8
                           join y in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport on x.MonthReportId equals y.MonthReportId
                           where y.ProjectId == projectId && y.ReporMonth.Value.Year == monthD.Value.Year && y.ReporMonth.Value.Month == monthD.Value.Month
        ...
        {
            var getInfo = from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport8Item
                          where x.MonthReportId == monthReportId
                          orderby x.UnitName
        ...
        public static Model.SeDinMonthReport9Item getSeDinMonthReport9ById(string projectId, string month)
        {
            ...
            if (monthD.HasValue)
            {
                getInfo = (from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport9
                           join y in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport on x.MonthReportId equals y.MonthReportId
                           where y.ProjectId == projectId && y.ReporMonth.Value.Year == monthD.Value.Year && y.ReporMonth.Value.Month == monthD.Value.Month
        ...
        {
            var getInfo = from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport9Item_Rectification
                          where x.MonthReportId == monthReportId
                          orderby x.UnitName
        ...
        {
            var getInfo = from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport9Item_Special
                          where x.MonthReportId == monthReportId
                          orderby x.TypeName
        ...
        {
            var getInfo = from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport9Item_Stoppage
                          where x.MonthReportId == monthReportId
                          orderby x.UnitName
        ...
        public static Model.SeDinMonthReport10Item getSeDinMonthReport10ById(string projectId, string month)
        {
            ...
            if (monthD.HasValue)
            {
                getInfo = (from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport10
                           join y in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport on x.MonthReportId equals y.MonthReportId
                           where y.ProjectId == projectId && y.ReporMonth.Value.Year == monthD.Value.Year && y.ReporMonth.Value.Month == monthD.Value.Month
        ...
        public static Model.SeDinMonthReport11Item getSeDinMonthReport11ById(string projectId, string month)
        {
            ...
            if (monthD.HasValue)
            {
                getInfo = (from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport11
                           join y in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport on x.MonthReportId equals y.MonthReportId
                           where y.ProjectId == projectId && y.ReporMonth.Value.Year == monthD.Value.Year && y.ReporMonth.Value.Month == monthD.Value.Month
        ...
        public static Model.SeDinMonthReport12Item getSeDinMonthReport12ById(string projectId, string month)
        {
            ...
            if (monthD.HasValue)
            {
                getInfo = (from x in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport12
                           join y in new Model.SGGLDB(Funs.ConnString).SeDin_MonthReport on x.MonthReportId equals y.MonthReportId
                           where y.ProjectId == projectId && y.ReporMonth.Value.Year == monthD.Value.Year && y.ReporMonth.Value.Month == monthD.Value.Month
