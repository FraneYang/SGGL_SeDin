...
namespace FineUIPro.Web.HSSE.Technique
{
    public partial class HazardList : PageBase
    {
        ...
        private List<Model.Technique_HazardListType> GetNewHazardListType(string parentId)
        {
            ...
            if (parentId == "0")
            {
                return (from x in new Model.SGGLDB(Funs.ConnString).Technique_HazardListType where (x.IsCompany == false || x.IsCompany == null) && x.SupHazardListTypeId == "0" && x.HazardListTypeName.Contains(hazardListTypeName) orderby x.HazardListTypeCode select x).ToList();
            }
            else
            {
                return (from x in new Model.SGGLDB(Funs.ConnString).Technique_HazardListType where (x.IsCompany == false || x.IsCompany == null) && x.SupHazardListTypeId == parentId orderby x.HazardListTypeCode select x).ToList();
            }
        }
        #endregion
        ...
        private void BindGrid()
        {
            ...
            {
                new SqlParameter("@HazardListTypeId", this.trHazardListType.SelectedNode.NodeID)
            };
