...
namespace FineUIPro.Web.HSSE.License
{
    public partial class LimitedSpace : PageBase
    {
        ...
        protected void btnPrinter_Click(object sender, EventArgs e)
        {
            ...
            initTemplatePath = "File\\Word\\HSSE\\受限空间作业票.doc";
            ...
            ///更新书签
            var getLimitedSpace = LicensePublicService.GetLimitedSpaceById(Id);
            ...
            if (bookmarkLicenseCode != null)
            {
                ...
                {
                    bookmarkLicenseCode.Text = getLimitedSpace.LicenseCode;
                }
            }
            ...
            if (bookmarkApplyManName != null)
            {
                if (getLimitedSpace != null)
                {
                    var getUser = UserService.GetUserByUserId(getLimitedSpace.ApplyManId);
                    if (getUser != null)
                    {
            ...
            if (bookmarkUnitName != null)
            {
                if (getLimitedSpace != null)
                {
                    if (!string.IsNullOrEmpty(getLimitedSpace.ApplyUnitId))
                    {
                        bookmarkUnitName.Text = BLL.UnitService.GetUnitNameByUnitId(getLimitedSpace.ApplyUnitId);
                    }
            ...
            if (bookmarkWorkPalce != null)
            {
                if (getLimitedSpace != null)
                {
                    bookmarkWorkPalce.Text = getLimitedSpace.WorkPalce;
            ...
            if (bookmarkFireWatchManName != null)
            {
                if (getLimitedSpace != null)
                {
                    var getUser = UserService.GetUserByUserId(getLimitedSpace.FireWatchManId);
                    if (getUser != null)
                    {
                    ...
                    {
                        bookmarkFireWatchManName.Text = getLimitedSpace.FireWatchManName;
                    }
            ...
            if (bookmarkValidityDate != null)
            {
                if (getLimitedSpace != null)
                {
                    if (getLimitedSpace.ValidityStartTime.HasValue)
                    {
                        ...
                        bookmarkValidityDate.Text = getLimitedSpace.ValidityStartTime.Value.Year + "年" + getLimitedSpace.ValidityStartTime.Value.Month + "月" + getLimitedSpace.ValidityStartTime.Value.Day + "日" + getLimitedSpace.ValidityStartTime.Value.Hour + "时至";
                        if (getLimitedSpace.ValidityEndTime.HasValue)
                        {
                            bookmarkValidityDate.Text += getLimitedSpace.ValidityEndTime.Value.Year + "年" + getLimitedSpace.ValidityEndTime.Value.Month + "月" + getLimitedSpace.ValidityEndTime.Value.Day + "日" + getLimitedSpace.ValidityEndTime.Value.Hour + "时";
                        }
                    }
                }
            }
            ...
            if (bookmarkWorkMeasures != null)
            {
                if (getLimitedSpace != null)
                {
                    bookmarkWorkMeasures.Text = getLimitedSpace.WorkMeasures;
            ...
            Bookmark bookmarkCance = doc.Range.Bookmarks["Cance"];//取消
            if (bookmarkCance != null)
            {
                if (getLimitedSpace != null)
                {
                    ...
                    {
                        var getUser = UserService.GetUserByUserId(getLimitedSpace.CancelManId);
                        if (getUser != null)
                        {
                            if (!string.IsNullOrEmpty(getUser.SignatureUrl) && File.Exists(rootPath + getUser.SignatureUrl))
                            {
                                var file = rootPath + getUser.SignatureUrl;
                                DocumentBuilder builders = new DocumentBuilder(doc);
                                builders.MoveToBookmark("Cance");
                                builders.InsertImage(file, 80, 20);
                                builders.Write("取消原因：" + getLimitedSpace.CancelReasons + "  取消时间：" + string.Format("{0:yyyy-MM-dd HH:mm}", getLimitedSpace.CancelTime));
                            }
                            else
                            {
                                bookmarkCance.Text = getUser.UserName + "  取消原因：" + getLimitedSpace.CancelReasons + "  取消时间：" + string.Format("{0:yyyy-MM-dd HH:mm}", getLimitedSpace.CancelTime);
                            }
                        }
                    }
                }
            ...
            if (bookmarkClose != null)
            {
                if (getLimitedSpace != null)
                {
                    ...
                    {
                        var getUser = UserService.GetUserByUserId(getLimitedSpace.CloseManId);
                        if (getUser != null)
                        {
                            if (!string.IsNullOrEmpty(getLimitedSpace.CloseReasons))
                            {
                                bookmarkClose.Text = getLimitedSpace.CloseReasons + " 关闭时间："
                            ...
                            else if (!string.IsNullOrEmpty(getUser.SignatureUrl) && File.Exists(rootPath + getUser.SignatureUrl))
                            {
                                ...
                                builders.InsertImage(file, 80, 20);
                                builders.Write("关闭时间：" + string.Format("{0:yyyy-MM-dd HH:mm}", getLimitedSpace.CloseTime));
                            }
                            else
                            {
                                bookmarkClose.Text = getUser.UserName + "  关闭时间：" + string.Format("{0:yyyy-MM-dd HH:mm}", getLimitedSpace.CloseTime);
                            }
                        }
                    }
                }
