...
namespace FineUIPro.Web.HSSE.License
{
    public partial class NightWork : PageBase
    {
        ...
        protected void btnPrinter_Click(object sender, EventArgs e)
        {
            ...
            initTemplatePath = "File\\Word\\HSSE\\夜间施工作业票.doc";
            ...
            ///更新书签
            var getNightWork = LicensePublicService.GetNightWorkById(Id);
            ...
            if (bookmarkLicenseCode != null)
            {
                ...
                {
                    bookmarkLicenseCode.Text = getNightWork.LicenseCode;
                }
            }
            ...
            if (bookmarkApplyManName != null)
            {
                if (getNightWork != null)
                {
                    var getUser = UserService.GetUserByUserId(getNightWork.ApplyManId);
                    if (getUser != null)
                    {
            ...
            if (bookmarkUnitName != null)
            {
                if (getNightWork != null)
                {
                    if (!string.IsNullOrEmpty(getNightWork.ApplyUnitId))
                    {
                        bookmarkUnitName.Text = BLL.UnitService.GetUnitNameByUnitId(getNightWork.ApplyUnitId);
                    }
            ...
            if (bookmarkWorkPalce != null)
            {
                if (getNightWork != null)
                {
                    bookmarkWorkPalce.Text = getNightWork.WorkPalce;
            ...
            if (bookmarkWorkMeasures != null)
            {
                if (getNightWork != null)
                {
                    bookmarkWorkMeasures.Text = getNightWork.WorkMeasures;
            ...
            if (bookmarkValidityDate != null)
            {
                if (getNightWork != null)
                {
                    if (getNightWork.ValidityStartTime.HasValue)
                    {
                        ...
                        bookmarkValidityDate.Text = getNightWork.ValidityStartTime.Value.Year + "年" + getNightWork.ValidityStartTime.Value.Month + "月" + getNightWork.ValidityStartTime.Value.Day + "日" + getNightWork.ValidityStartTime.Value.Hour + "时至";
                        if (getNightWork.ValidityEndTime.HasValue)
                        {
                            bookmarkValidityDate.Text += getNightWork.ValidityEndTime.Value.Year + "年" + getNightWork.ValidityEndTime.Value.Month + "月" + getNightWork.ValidityEndTime.Value.Day + "日" + getNightWork.ValidityEndTime.Value.Hour + "时";
                        }
                    }
                }
            }
            ...
            if (bookmarkWorkLeaderTel != null)
            {
                if (getNightWork != null)
                {
                    var getUser = UserService.GetUserByUserId(getNightWork.WorkLeaderId);
                    if (getUser != null)
                    {
                            bookmarkWorkLeaderTel.Text = getUser.UserName + "/" + getNightWork.WorkLeaderTel;
                    }

                }
            }
            Bookmark bookmarkSafeLeaderTel = doc.Range.Bookmarks["SafeLeaderTel"];//安全负责人
            if (bookmarkSafeLeaderTel != null)
            {
                if (getNightWork != null)
                {
                    var getUser = UserService.GetUserByUserId(getNightWork.SafeLeaderId);
                    if (getUser != null)
                    {
                            bookmarkSafeLeaderTel.Text = getUser.UserName + "/" + getNightWork.SafeLeaderTel;
                    }
            ...
            if (bookmarkCance != null)
            {
                if (getNightWork != null)
                {
                    ...
                    {
                        var getUser = UserService.GetUserByUserId(getNightWork.CancelManId);
                        if (getUser != null)
                        {
                            if (!string.IsNullOrEmpty(getUser.SignatureUrl) && File.Exists(rootPath + getUser.SignatureUrl))
                            {
                                ...
                                builders.InsertImage(file, 80, 20);
                                builders.Write("取消原因：" + getNightWork.CancelReasons + "  取消时间：" + string.Format("{0:yyyy-MM-dd HH:mm}", getNightWork.CancelTime));
                            }
                            else
                            {
                                bookmarkCance.Text = getUser.UserName + "  取消原因：" + getNightWork.CancelReasons + "  取消时间：" + string.Format("{0:yyyy-MM-dd HH:mm}", getNightWork.CancelTime);
                            }
                        }
                    }
                }
                    ...
                    if (!string.IsNullOrEmpty(getNightWork.CloseManId))
                    {
                        ...
                        if (getUser != null)
                        {
                            ...
                            else if (!string.IsNullOrEmpty(getUser.SignatureUrl) && File.Exists(rootPath + getUser.SignatureUrl))
                            {
                                ...
                                builders.InsertImage(file, 80, 20);
                                builders.Write("关闭时间：" + string.Format("{0:yyyy-MM-dd HH:mm}", getNightWork.CloseTime));
                            }
                            else
                            {
                                bookmarkClose.Text = getUser.UserName + "  关闭时间：" + string.Format("{0:yyyy-MM-dd HH:mm}", getNightWork.CloseTime);
                            }
                        }
                    }
                }
