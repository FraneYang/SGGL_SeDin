...
namespace FineUIPro.Web.BaseInfo
{
    public partial class TrainLevel : PageBase
    {
        ...
        {
            var q = from x in new Model.SGGLDB(Funs.ConnString).Base_TrainLevel orderby x.TrainLevelCode select x;
        ...
        {
            List<Model.Base_TrainLevel> source = (from x in new Model.SGGLDB(Funs.ConnString).Base_TrainLevel orderby x.TrainLevelCode select x).ToList();
        ...
        {
            var q = new Model.SGGLDB(Funs.ConnString).Base_TrainLevel.FirstOrDefault(x => x.TrainLevelCode == this.txtTrainLevelCode.Text.Trim() && (x.TrainLevelId != hfFormID.Text || (hfFormID.Text == null && x.TrainLevelId != null)));
            if (q != null)
            {
                ShowNotify("输入的培训级别编号已存在！", MessageBoxIcon.Warning);
            }

            var q2 = new Model.SGGLDB(Funs.ConnString).Base_TrainLevel.FirstOrDefault(x => x.TrainLevelName == this.txtTrainLevelName.Text.Trim() && (x.TrainLevelId != hfFormID.Text || (hfFormID.Text == null && x.TrainLevelId != null)));
            if (q2 != null)
            {
