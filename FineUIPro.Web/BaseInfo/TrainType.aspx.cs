...
namespace FineUIPro.Web.BaseInfo
{
    public partial class TrainType : PageBase
    {
        ...
        {
            var q = from x in new Model.SGGLDB(Funs.ConnString).Base_TrainType orderby x.TrainTypeCode select x;
        ...
        {
            List<Model.Base_TrainType> source = (from x in new Model.SGGLDB(Funs.ConnString).Base_TrainType orderby x.TrainTypeCode select x).ToList();
        ...
        {
            var q = new Model.SGGLDB(Funs.ConnString).Base_TrainType.FirstOrDefault(x => x.TrainTypeCode == this.txtTrainTypeCode.Text.Trim() && (x.TrainTypeId != hfFormID.Text || (hfFormID.Text == null && x.TrainTypeId != null)));
            if (q != null)
            {
                ShowNotify("输入的类型编号已存在！", MessageBoxIcon.Warning);
            }

            var q2 = new Model.SGGLDB(Funs.ConnString).Base_TrainType.FirstOrDefault(x => x.TrainTypeName == this.txtTrainTypeName.Text.Trim() && (x.TrainTypeId != hfFormID.Text || (hfFormID.Text == null && x.TrainTypeId != null)));
            if (q2 != null)
            {
        ...
        private bool judgementDelete(string id, bool isShow)
        {
            ...
            //if (new Model.SGGLDB(Funs.ConnString).EduTrain_TrainRecord.FirstOrDefault(x => x.TrainTypeId == id) != null)
            //{
