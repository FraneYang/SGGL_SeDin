...
{
    public partial class ContractAgreement : PageBase
    {
        #region 加载
        /// <summary>
        /// 加载页面
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                //btnNew.OnClientClick = Window1.GetShowReference("ContractAgreementEdit.aspx", "合同协议书") + "return false;";
                GetButtonPower();
                BindGrid();
            }
        }
        ...
        private void EditData()
        {
            ...
            var subcontractAgreement = BLL.SubcontractAgreementService.GetSubcontractAgreementByContractId(id);
            if (subcontractAgreement != null)
            {
                PageContext.RegisterStartupScript(Window1.GetShowReference(String.Format("ContractAgreementEdit.aspx?SubcontractAgreementId={0}&&ContractId={1}", subcontractAgreement.SubcontractAgreementId, id, "编辑 - ")));
            }
            else
            {
                PageContext.RegisterStartupScript(Window1.GetShowReference(String.Format("ContractAgreementEdit.aspx?ContractId={0}", id, "编辑 - ")));
            }
        }
        #endregion 
